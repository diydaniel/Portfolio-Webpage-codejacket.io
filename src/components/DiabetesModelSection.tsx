...
export default function DiabetesModelSection() {
    ...
    async function loadArtifacts() {
      try {
        const [metricsRes, fiRes] = await Promise.all([
          fetch("/diabetes-metrics.json"),
          fetch("/diabetes-feature-importance.json"),
        ]);
        ...
        console.error("Error loading diabetes model artifacts:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (!metrics) {
    return (
      ...
      >
        <p>Could not load diabetes model metrics.</p>
      </section>
    );
  }
      ...
      <div className="max-w-5xl mx-auto px-4 space-y-10">
        <header className="space-y-3">
          <h2 className="text-2xl md:text-3xl font-bold">
            Diabetes Progression – ML Predictive Pipeline
          </h2>
          <p className="text-sm md:text-base text-slate-300 max-w-2xl">
            A Random Forest regression model trained on the scikit-learn
            diabetes dataset. This section reads JSON artifacts generated by
